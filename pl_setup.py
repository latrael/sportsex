...
def main():
    ...
    for r in required:
        if r not in df.columns:
            print(
                f"[WARN] Missing column: {r}. Creating default zeros.", file=sys.stderr
            )
            df[r] = 0
    ...
    numeric_candidates = [
        c for c in df.columns if c not in ["player", "club", "nationality", "position"]
    ]
    ...
    count_like = [
        ...
        "carries",
        "progressive_carries",
        "carries_ended_with_goal",
        "carries_ended_with_assist",
        "carries_ended_with_shot",
        "carries_ended_with_chance",
        "possession_won",
        "dispossessed",
        "clearances",
        "interceptions",
    ...
    df["mins_per_app"] = np.where(
        df["appearances"] > 0, df["minutes"] / df["appearances"], 0.0
    )
    df["availability"] = df["minutes"]
    ...
    role_weights = {
        "ST": {
            ...
            "shots_per90": 0.10,
            "carries_ended_with_shot_per90": 0.15,
            "gduels_won": 0.10,
            "gduels_pct": 0.10,
        },
        "W": {
            ...
            "successful_crosses_per90": 0.20,
            "crosses_pct": 0.10,
            "dispossessed_per90": -0.10,
        },
        "AM": {
            "assists_per90": 0.30,
            "through_balls_per90": 0.20,
            "progressive_carries_per90": 0.20,
            "passes_completed": 0.10,
            "pass_pct": 0.20,
        },
        "CM": {
            "progressive_carries_per90": 0.20,
        ...
        "GK": {
            "saves_pct": 0.35,
            "goals_prevented": 0.30,
            "goals_conceded_per90": -0.25,
            "xgot_conceded_per90": -0.10,
            "high_claims_per90": 0.10,
            "punches_per90": 0.05,
        },
    ...
    cols_out += [
        c
        for c in df.columns
        if c.endswith("_per90") or c.endswith("_z") or c.endswith("_pctile")
    ]
